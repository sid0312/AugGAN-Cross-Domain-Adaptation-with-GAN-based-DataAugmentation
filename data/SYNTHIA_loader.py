...
class ImageDataset(dset.ImageFolder):
    def __init__(self, root_dir, style_A, style_B, load_size, seqs, fine_size, phase, opt_choice):
        ...
        if self.phase == "train":
            self.A_paths = self.A_paths[:int(0.5*self.A_size)]
            self.A_label_paths = self.A_label_paths[:int(0.5*self.A_size)]
            self.B_paths = self.B_paths[:int(0.5*self.B_size)]
            self.B_label_paths = self.B_label_paths[:int(0.5*self.B_size)]
        elif self.phase == "val":
            self.A_paths = self.A_paths[int(0.5*self.A_size): int(0.75*self.A_size)]
            self.A_label_paths = self.A_label_paths[int(0.5*self.A_size): int(0.75*self.A_size)]
            self.B_paths = self.B_paths[int(0.5*self.B_size): int(0.75*self.B_size)]
            self.B_label_paths = self.B_label_paths[int(0.5*self.B_size): int(0.75*self.B_size)]
        else:
            self.A_paths = self.A_paths[int(0.75*self.A_size):]
            self.A_label_paths = self.A_label_paths[int(0.75*self.A_size):]
            self.B_paths = self.B_paths[int(0.75*self.B_size):]
            self.B_label_paths = self.B_label_paths[int(0.75*self.B_size):]
...
#        self.B_size = round(self.B_size * 0.05)
    ...
    def __getitem__(self, index):
        ...
        A_label = cv2.imread(A_label_path)
        B_label = cv2.imread(B_label_path)
        A_label = cv2.cvtColor(A_label, cv2.COLOR_BGR2GRAY)
        B_label = cv2.cvtColor(B_label, cv2.COLOR_BGR2GRAY)
...
def get_dataset(seqs, style, root_dir, img=True, all_=True):
    ...
    if img:
        ...
        tail_F = '/GT/LABELS/Stereo_Left/Omni_F'
        tail_B = '/GT/LABELS/Stereo_Left/Omni_B'
        tail_L = '/GT/LABELS/Stereo_Left/Omni_L'
        if all_:
            tail_R = '/GT/LABELS/Stereo_Left/Omni_R'
        else:
            tail_R = None

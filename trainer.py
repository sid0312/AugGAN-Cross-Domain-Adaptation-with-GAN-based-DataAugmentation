...
class trainer():
    ...
    def train(self, train_data_loader, val_data_loader, epochs, test_data_loader=None):
        for epoch in range(1, epochs+1):
            ...
            epoch_stats = '---> Epoch# {} summary loss g_x:{loss_g_x:.4f}, g_y:{loss_g_y:.4f}, d_x:{loss_d_x:.4f}, d_y:{loss_d_y:.4f}'.format(self.epoch, **metrics)      
    ...
    def evaluate(self, data_loader, test_on_cpu=False):
        self.network.enc_x.eval()
        self.network.enc_y.eval()
        self.network.mul_gen_x.eval()
        self.network.mul_gen_y.eval()
        ...
        self.network.dis_y.eval()
        X, Rec_X, Y, Rec_Y = [], [], [], []
        fid = FrechetInceptionDistance(feature=64, compute_on_cpu=test_on_cpu)
        device = torch.device("cpu") if test_on_cpu else torch.device("cuda") 
        with torch.no_grad():
            progress = tqdm.tqdm(data_loader)
            ...
            fid.update(torch.cat(X, 0).to(dtype=torch.uint8, device=device), real=True)
            fid.update(torch.cat(Rec_X, 0).to(dtype=torch.uint8, device=device), real=False)
            x_fid = fid.compute()
            fid.reset()
            
            
            fid.update(torch.cat(Y, 0).to(dtype=torch.uint8, device=device), real=True)
            fid.update(torch.cat(Rec_Y, 0).to(dtype=torch.uint8, device=device), real=False)
            y_fid = fid.compute()
            fid.reset()

...
#        self.optimizer_E.step()
    ...
    def backward_generator(self, backprop=True):    
        ...
        self.rec_x, self.parse_x_2 = self.mul_gen_y(self.enc_y(self.fake_y))
        self.loss_cycle_x = self.criterionCyC(self.rec_x, self.real_x) * self.lambda_A 
        
        
        self.rec_y, self.parse_y_2 = self.mul_gen_x(self.enc_x(self.fake_x))
    ...
    def generate_output(self):
        y_fake, _ = self.mul_gen_x(self.enc_x(self.real_x))
        x_fake, _ = self.mul_gen_y(self.enc_y(self.real_y))
        x_rec, _ = self.mul_gen_y(self.enc_y(y_fake))
        y_rec, _ = self.mul_gen_x(self.enc_x(x_fake))
